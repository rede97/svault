...
┌─────────────────────────────────────────────────────────┐
│  Stage A: 目录扫描（1 线程）                             │
│  VfsBackend::walk 递归遍历源目录，扩展名过滤在 walk 内完成│
│  SystemFs 使用 jwalk（rayon 并行 readdir）                │
...
- **Stage B** 在源端执行，每次只读 64KB，网络/MTP 场景下快速给用户反馈，减少不必要的文件传输
...
用户可通过 `-R`/`--recheck` 对 `likely_duplicate` 文件执行二次验证：

| 模式 | 行为 | IO 代价 |
|------|------|---------|
| `-R fast`（默认） | 信任 CRC32C 缓存，不执行二次验证 | 零 |
| `-R exif` | 从归档侧读取 EXIF 头部（本地，快），与源文件 EXIF 二进制比对 | 仅 64KB × 2 |
| `-R hash` | 计算源文件强哈希，与数据库已有哈希比对 | 全文件读取（网络场景代价高） |

二次验证发现不同 → 重新标记为 `likely_new` → 进入传输队列。
...
Skip   DCIM/Canon/IMG_003.CR3   17.1 MiB  (duplicate)   # 仅 --show-skip 时显示
