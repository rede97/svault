...
> `.svault/sessions/import/<ts-id>/staging/`，入库 commit 后才 rename
> 到最终路径，半成品不再污染最终路径（见下文 Stage C/E 与"原子性与可恢复性"）；
> 复制前落 plan.json 记录意图；manifest 原子写入会话目录。

### Stage C：复制阶段（staging 原子提交，2026-08-09 起）

复制前先原子写 `plan.json`（src/dest/size/crc32c 的操作意图，fail-fast），
然后文件复制到 `.svault/sessions/import/<ts-id>/staging/`（镜像最终相对路径），
...
由本进程 Stage E 后清理 staging 子树，不再残留于最终路径。

### Stage E：数据库写入 + 原子提交

批量事务入库（记录的是**最终路径**），写入 manifest
（`.svault/sessions/<kind>/<ts-id>/manifest.json`，原子写入）；事务提交成功后才将暂存文件
逐个原子 rename 到最终路径（`fs::atomic_commit`：rename + 目录 fsync）。

**不变量：最终路径可见 ⟹ 已完整复制 + 哈希 + 入库。**
commit 后、rename 前进程被杀时，文件留在 staging 且 DB 有记录；下次
import 启动时 `session::reconcile` 对账：有 `imported` 记录的补 rename，
其余中断残留**只报告不删**（`Hint::SessionResidue` 带目录/文件数/字节数），
用户审阅 plan.json 后手动处理。
...
Import operation completed
  ...
  Manifest: .svault/sessions/import/20240315T143000-a1b2c/manifest.json
```

### 原子性与可恢复性（2026-08-09 重写）

- 复制进入 `.svault/sessions/import/<ts-id>/staging/`，DB 整批单事务写入
  （WAL 模式），未提交自动回滚；**commit 成功后才 rename 到最终路径**
  （tmp→fsync→hash→入库→rename，staging 原子提交）。
- 不变量：**最终路径可见 ⟹ 已完整复制 + 哈希 + 入库**；中断的半成品永不
  出现在用户可见目录树。
- 中断后重新执行导入命令：
  - 已入库文件：CRC32C 缓存命中秒级跳过；
  - "已入库未 rename"：启动对账（`session::reconcile`）补 rename；
  - "已复制未入库"：对账**只报告不删**（Hint 带目录/文件数/字节数），
    用户审阅 plan.json 后手动处置，重跑重新复制（不做断点续传，
