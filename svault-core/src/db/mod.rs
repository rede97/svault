        ...
        for row in rows {
            ...
            if ev.prev_hash != prev_hash {
                return Err(rusqlite::Error::InvalidParameterName(format!(
                    "chain broken at seq {}: prev_hash mismatch",
                    ev.seq
                )));
            }
            ...
            if ev.self_hash != expected {
                return Err(rusqlite::Error::InvalidParameterName(format!(
                    "chain broken at seq {}: self_hash mismatch",
                    ev.seq
                )));
            }
...
mod tests {
    ...
    fn db_open_in_memory_is_isolated() {
        ...
        // Add event to db1
        db1.append_event("test", "file", 1, r#"{"path":"test.txt"}"#, |_conn| Ok(()))
            .unwrap();
    ...
    fn append_event_creates_valid_chain() {
        ...
        // Append second event
        db.append_event("import", "file", 2, r#"{"path":"b.jpg"}"#, |_conn| Ok(()))
            .unwrap();
    ...
    fn verify_chain_detects_tampering() {
        ...
        // Add an event
        db.append_event("import", "file", 1, r#"{"path":"a.jpg"}"#, |_conn| Ok(()))
            .unwrap();
    ...
    fn get_events_returns_events_in_descending_order() {
        ...
        db.append_event("import", "file", 1, "{}", |_conn| Ok(()))
            .unwrap();
        std::thread::sleep(std::time::Duration::from_millis(10));
        db.append_event("import", "file", 2, "{}", |_conn| Ok(()))
            .unwrap();
        std::thread::sleep(std::time::Duration::from_millis(10));
        db.append_event("update", "file", 1, "{}", |_conn| Ok(()))
            .unwrap();
    ...
    fn get_events_respects_limit() {
        ...
        for i in 1..=5 {
            db.append_event("import", "file", i, "{}", |_conn| Ok(()))
                .unwrap();
        }
